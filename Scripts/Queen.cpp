...
Queen::Queen(int id, int x, int y) : Warrior(id, 15, 20, 25, x, y) {}

const string Queen::getType() const
{
    return "Queen";
}

void Queen::attack(Team &enemies)
{
    int ogX, ogY;
...
void Queen::move()
{
    ...
    if (optionMoves.size() > 0)
    {
        ...
        if (it != optionMoves.end())
        {
            ...
            cout << "Oueen " << " at (" << ogX << ", " << ogY << ") moves to (" << Warrior::x << ", " << Warrior::y << ")." << endl;
        }
    }
    else
    {
